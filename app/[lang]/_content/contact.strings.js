// Every translated string on the contact page, one entry per key per language.
// The markup lives in contact.jsx and is shared by all three, so a layout
// change is made once instead of being copied into three files that nothing
// keeps in step.
//
// Routes are deliberately NOT here — they are derived from `lang` via
// pagePath(), so a URL change cannot leave one language on a stale path.
export const STRINGS = {
  en: {
    home: "Home",
    contact: "Contact",
    yourTrustedPartnerInEngineering: "Your Trusted Partner in Engineering Solutions",
    readyToCollaborate: "Ready to Collaborate?",
    reachOutWithYourInquiries: "Reach out with your inquiries or RFQs. Include the scope, timeline, and budget, and we’ll respond with the right team.",
    quoteFormTitle: "Request a Quote",
    quoteFormDescription: "Tell us what you need and we’ll open a ready-to-send email in your own mail client — nothing is submitted from this page.",
    quoteFormName: "Name",
    quoteFormCompany: "Company",
    quoteFormOptional: "optional",
    quoteFormScope: "What do you need?",
    quoteFormScopePlaceholder: "e.g. Electrical design for a 10MW substation upgrade",
    quoteFormTimeline: "Timeline",
    quoteFormTimelinePlaceholder: "e.g. Q2 2027, or flexible",
    quoteFormBudget: "Budget range",
    quoteFormBudgetPlaceholder: "e.g. $50k–100k, or not yet determined",
    quoteFormSubmit: "Send via Email",
    quoteFormSubject: "RFQ from",
    contactDetails: "Contact Details",
    email2: "Email",
    phone: "Phone",
    address: "Address",
    mardinStreetOppositeDilanCity: "Mardin Street, Opposite Dilan City, Sulaymaniyah (46001), Iraq",
    supplyChainTeam: "Supply Chain Team",
    technicalTeam: "Technical Team",
    email3: "Email",
    findUs: "Find Us",
    sulaymaniyahKurdistanRegionOfIraq: "Sulaymaniyah, Kurdistan Region of Iraq",
  },
  ar: {
    home: "الرئيسية",
    contact: "تواصل معنا",
    yourTrustedPartnerInEngineering: "شريككم الموثوق في الحلول الهندسية",
    readyToCollaborate: "مستعد للتعاون؟",
    reachOutWithYourInquiries: "تواصل معنا لأي استفسار أو طلب عرض سعر — أخبرنا بالنطاق والجدول الزمني والميزانية، وسنرد عليك بالفريق المناسب.",
    quoteFormTitle: "اطلب عرض سعر",
    quoteFormDescription: "أخبرنا بما تحتاجه وسنفتح لك رسالة إلكترونية جاهزة للإرسال من برنامج بريدك الخاص — لا يتم إرسال أي شيء من هذه الصفحة.",
    quoteFormName: "الاسم",
    quoteFormCompany: "الشركة",
    quoteFormOptional: "اختياري",
    quoteFormScope: "ما الذي تحتاجه؟",
    quoteFormScopePlaceholder: "مثال: تصميم كهربائي لترقية محطة تحويل بقدرة 10 ميغاواط",
    quoteFormTimeline: "الجدول الزمني",
    quoteFormTimelinePlaceholder: "مثال: الربع الثاني من 2027، أو مرن",
    quoteFormBudget: "نطاق الميزانية",
    quoteFormBudgetPlaceholder: "مثال: 50-100 ألف دولار، أو لم يُحدد بعد",
    quoteFormSubmit: "إرسال عبر البريد الإلكتروني",
    quoteFormSubject: "طلب عرض سعر من",
    contactDetails: "معلومات التواصل",
    email2: "البريد الإلكتروني",
    phone: "الهاتف",
    address: "العنوان",
    mardinStreetOppositeDilanCity: "شارع ماردين، مقابل مدينة دلان، السليمانية (46001)، العراق",
    supplyChainTeam: "فريق سلسلة التوريد",
    technicalTeam: "الفريق الفني",
    email3: "البريد الإلكتروني",
    findUs: "موقعنا",
    sulaymaniyahKurdistanRegionOfIraq: "السليمانية، إقليم كوردستان العراق",
  },
  ku: {
    home: "سەرەکی",
    contact: "پەیوەندیمان پێوە بکە",
    yourTrustedPartnerInEngineering: "هاوبەشی متمانەپێکراوتان لە چارەسەری ئەندازیاریدا",
    readyToCollaborate: "ئامادەیت بۆ هاوکاری؟",
    reachOutWithYourInquiries: "پەیوەندیمان پێوە بکە بۆ هەر پرسیارێک یان داواکاری نرخنامە — مەودا، کاتبەندی و بودجەکەت پێمان بڵێ، ئێمەش بە تیمی گونجاو وەڵامت دەدەینەوە.",
    quoteFormTitle: "داوای نرخنامە بکە",
    quoteFormDescription: "پێمان بڵێ چیت پێویستە و ئیمەیلێکی ئامادە بۆ ناردن لە بەرنامەی ئیمەیلی خۆتدا دەکەینەوە — هیچ شتێک لەم پەڕەیەوە نانێردرێت.",
    quoteFormName: "ناو",
    quoteFormCompany: "کۆمپانیا",
    quoteFormOptional: "دڵخوازانە",
    quoteFormScope: "چیت پێویستە؟",
    quoteFormScopePlaceholder: "بۆ نموونە: دیزاینی کارەبایی بۆ نوێکردنەوەی وێستگەیەکی گواستنەوەی ١٠ مێگاواتی",
    quoteFormTimeline: "کاتبەندی",
    quoteFormTimelinePlaceholder: "بۆ نموونە: چارەکی دووەمی ٢٠٢٧، یان گونجاو",
    quoteFormBudget: "مەودای بودجە",
    quoteFormBudgetPlaceholder: "بۆ نموونە: ٥٠-١٠٠ هەزار دۆلار، یان هێشتا دیارینەکراوە",
    quoteFormSubmit: "ناردن لە ڕێگەی ئیمەیلەوە",
    quoteFormSubject: "داواکاری نرخنامە لە",
    contactDetails: "زانیاریی پەیوەندی",
    email2: "ئیمەیل",
    phone: "تەلەفۆن",
    address: "ناونیشان",
    mardinStreetOppositeDilanCity: "شەقامی ماردین، بەرامبەر شاری دیلان، سلێمانی (46001)، عێراق",
    supplyChainTeam: "تیمی زنجیرەی دابینکردن",
    technicalTeam: "تیمی تەکنیکی",
    email3: "ئیمەیل",
    findUs: "شوێنمان",
    sulaymaniyahKurdistanRegionOfIraq: "سلێمانی، هەرێمی کوردستانی عێراق",
  },
};
